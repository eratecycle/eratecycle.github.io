...
module.exports = Backbone.View.extend({
  ...
  onShow: function() {
    ...
    var managedServices = this.model.transactions.where({description: 'Managed Services'});
    var misc = this.model.transactions.where({description: 'Misc and Equipment Charges'});
    ...
    var managedServicesTotal = _.reduce(managedServices, function(memo, trans){
      return memo + trans.get('amount');
    }, 0).toFixed(2);

    var miscTotal = _.reduce(misc, function(memo, trans){
      return memo + trans.get('amount');
    }, 0).toFixed(2);
    ...
    var doughnutData = [
      ...
      {
        value: managedServicesTotal,
        color: "rgba(126,79,148,0.5)",
        label: "Managed Services"
      },
      {
        value: miscTotal,
        color: "rgba(26,179,48,0.5)",
        label: "Misc and Equipment Charges"
      }
    ];
